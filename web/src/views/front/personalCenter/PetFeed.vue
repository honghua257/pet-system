...
function getPageList(){
  console.log('PetFeed: 正在获取上门喂养订单数据...')
  const params = Object.assign({}, toRaw(pageInfo.value))
  if (selectedStatus.value) {
    params.status = selectedStatus.value
  }
  request.get('/petFeed/page', { params }).then(res => {
    if(!res){
      console.log('PetFeed: API响应为空')
      return
    }
    console.log('PetFeed: 获取到数据', res.data)
    listData.value = res.data?.list || []
    pageInfo.value.total = res.data?.total || 0
    console.log('PetFeed: 数据加载完成，条数:', listData.value.length)
  }).catch(error => {
    console.error('PetFeed: API请求失败', error)
  })
}
...
function deleteOne(row){
  ElMessageBox.confirm(`此操作将永久删除该订单, 是否继续?`, '提示', {
    ...
    request.delete('/petFeed/delete', { data: [row.id] }).then(res => {
      if(!res){ return }
...
/* 确保表格可见 */
.feed-container :deep(.el-table) {
  width: 100% !important;
  min-height: 300px;
}

.feed-container :deep(.el-table__body-wrapper) {
  min-height: 200px;
}

.feed-container :deep(.el-table__empty-block) {
  min-height: 150px;
}
